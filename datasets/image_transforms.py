...
get_train_img_tfms = lambda input_res: transforms.Compose(
    [
        transforms.Resize(int(input_res * 1.14), interpolation=Image.BICUBIC),
        transforms.CenterCrop(input_res),
        transforms.Normalize(
            (0.369, 0.395, 0.376), (0.205, 0.205, 0.212)
        ),
    ]
)
...
get_test_img_tfms = lambda input_res: transforms.Compose(
    [
        transforms.Resize(int(input_res * 1.14), interpolation=Image.BICUBIC),
        transforms.CenterCrop(input_res),
        transforms.Normalize((0.369, 0.395, 0.376), (0.205, 0.205, 0.212)),
    ]
)
